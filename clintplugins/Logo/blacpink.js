module.exports = async (context) => {
  ...
  if (!text || text.trim() === '') {
    return m.reply(`Yo, brain-dead moron, give me some text for the Blackpink Logo! Use *!blackpinklogo SomeText* or fuck off! 😡`);
  }
  ...
  try {
    ...
    if (data && data.success && data.result && data.result.image_url) {
      const caption = `Here’s your damn *Blackpink Logo*, ${m.pushName}! Don’t waste my time again, you K-pop wannabe prick! 😤\n` +
                     `📸 *Text*: ${cleanedText}\n` +
      ...
      await m.reply(`API’s being a bitch, no Blackpink Logo for you, loser! Try again later. 😒`);
    }
  } catch (error) {

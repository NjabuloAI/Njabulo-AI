module.exports = async (context) => {
  ...
  if (!text || text.trim() === '') {
    return m.reply(`Yo, brain-dead moron, give me some text for the Glitch Text logo! Use *!glitchtext SomeText* or fuck off! 😡`);
  }
  ...
  try {
    ...
    if (data && data.success && data.result && data.result.image_url) {
      const caption = `Here’s your damn *Glitch Text* logo, ${m.pushName}! Don’t waste my time again, you prick! 😤\n` +
                     `📸 *Text*: ${cleanedText}\n` +
      ...
      await m.reply(`API’s being a bitch, no Glitch Text logo for you, loser! Try again later. 😒`);
    }
  } catch (error) {

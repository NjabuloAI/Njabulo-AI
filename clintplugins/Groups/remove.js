...
module.exports = async (context) => {
  await middleware(context, async () => {
    ...
    if (!m.quoted && (!m.mentionedJid || m.mentionedJid.length === 0)) {
      return m.reply(`Yo, dumbass, mention a user or quote their message! Don’t make me guess.`);
    }
    ...
    if (!users) {
      ...
      return m.reply(`What the hell? No user found. Tag or quote someone, idiot.`);
    }
    ...
    if (
      ...
      return m.reply(`Invalid user format. Tag a valid user, you moron.`);
    }
    ...
    if (!parts) {
      ...
      return m.reply(`Something’s fucked up with that user’s ID. Try again, idiot.`);
    }

    // Prevent kicking the bot itself
    if (users === botNumber) {
      return m.reply(`Nice try, loser, you can’t kick me! I’m the boss here. 🦄`);
    }

...
if (!fs.existsSync(baseDir)) {
    try {
        ...
        console.error('Njabulo AI Store Error: Failed to create /app/message_data directory:', e);
    }
}
...
function saveChatData(remoteJid, messageId, chatData) {
    ...
    if (!fs.existsSync(chatDir)) {
        try {
            ...
            console.error('Njabulo AI Store Error: Failed to create directory:', e);
        }
    }
    ...
    try {
        ...
        console.error('Njabulo AI Store Error: Failed to save chat data:', error);
    }
}
...
function cleanupOldMessages(maxAgeMs = 24 * 60 * 60 * 1000) {
    try {
            ...
            fs.readdirSync(chatDir).forEach(file => {
                ...
                try {
                    ...
                    if (now - stats.mtimeMs > maxAgeMs) {
                        ...
                        // console.log(`Njabulo Jb Store Cleanup: Deleted old file ${filePath}`);
                    }
                } catch (err) {
                    console.error('Njabulo AI Store Cleanup Error:', err);
                }
            });
        ...
        console.error('Njabulo AI Store Cleanup Fatal Error:', err);
    }
}
